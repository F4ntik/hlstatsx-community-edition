...
def test_assignment_is_sticky_while_daemon_is_available() -> None:
    ...
    first = balancer.assign_server(
        "192.168.1.10:27015", timestamp=datetime(2024, 1, 1, tzinfo=timezone.utc)
    )
    assert first is not None

    reassigned = balancer.assign_server(
        "192.168.1.10:27015", timestamp=datetime(2024, 1, 1, 0, 0, 5, tzinfo=timezone.utc)
    )
...
def test_reassignment_when_daemon_marked_down() -> None:
    ...
    first = balancer.assign_server(
        "example:27015", timestamp=datetime(2024, 1, 1, tzinfo=timezone.utc)
    )
    assert first is not None
    ...
    reassigned = balancer.assign_server(
        "example:27015", timestamp=datetime(2024, 1, 1, 0, 1, tzinfo=timezone.utc)
    )
