...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--replay-mode",
        action="store_true",
        help="Enable replay-safe policy for awards and GeoIP processing",
    )
    parser.add_argument(
        "--version",
        "-v",
...
def load_settings(argv: Sequence[str] | None = None) -> RuntimeSettings:
    ...
    policy = RuntimePolicy.REPLAY_SAFE if options.replay_mode else RuntimePolicy.STRICT
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if settings.cli.version:
        print("hlstats-awards.py (Python port)")
        return 0

    from .calculator import AwardsCalculator, build_database_config
    ...
    calculator = AwardsCalculator(adapter, verbose=settings.cli.verbose)
    try:
        adapter.connect()
