...
$lines += "geoip_flag_rows=$(Invoke-Query "SELECT COUNT(*) FROM hlstats_Players WHERE lastAddress <> '' AND flag <> '';")"
$lines += "geoip_country_rows=$(Invoke-Query "SELECT COUNT(*) FROM hlstats_Players WHERE lastAddress <> '' AND country <> '';")"
$lines += "geoip_city_rows=$(Invoke-Query "SELECT COUNT(*) FROM hlstats_Players WHERE lastAddress <> '' AND city <> '';")"
$lines += "geoip_state_rows=$(Invoke-Query "SELECT COUNT(*) FROM hlstats_Players WHERE lastAddress <> '' AND state <> '';")"
$lines += "geoip_coordinates_rows=$(Invoke-Query "SELECT COUNT(*) FROM hlstats_Players WHERE lastAddress <> '' AND lat IS NOT NULL AND lng IS NOT NULL;")"
$lines += "maintenance_use_timestamp=$(Invoke-Query "SELECT value FROM hlstats_Options WHERE keyname='UseTimestamp';")"
$lines += ""
