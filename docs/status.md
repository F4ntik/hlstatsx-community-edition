...
- 2026-09-12, динамика игрока: вместо готовой PNG основной вид строится в браузере
  по последним 30 записям. Компактный серый график сохраняет две цветные линии,
  показывает обе величины при наведении и поддерживает клавиатуру. Даты содержат год,
  шкалы используют круглые деления; таблица доступна без JavaScript.
  [Устройство и границы изменения](charts-modern.md). В подписи добавлен отступ под ником.
  Стандартные действия переведены также в общем списке и на странице действия:
  «Хедшотов», «Появлений с бомбой», «Двойных убийств»; у русских счётчиков убран суффикс «раз».
  Проверены страницы на локальном стенде, наведение и выбор даты с клавиатуры.
  Исходники движка, обработка журналов и схема базы не менялись.

- 2026-09-12, карточка игрока: переведены стандартные действия и названия команд
  Counter-Strike во вкладке «Команды и действия». Перевод выполняется при отображении
  соответствующих колонок; названия в базе, пользовательские действия и ники не меняются.
  На игроке 187 проверены 13 действий и обе команды в русском интерфейсе.

- 2026-09-12, форумные подписи: обнаружены нечитаемые русские строки статистики
  и выход текста за границы изображения. Исправление использует шрифт с поддержкой
  кириллицы и измерение ширины строк; формат 400 × 75 и существующие фоны сохраняются.
  В подписи используется обозначение HLstatsX PY для линии выпуска `hlstatsx_py`.

- Локальный стенд 8394: восстановлено сохранение графиков в `hlstatsimg/progress`.
  Причиной сломанного изображения была папка только для чтения; выделен записываемый
  временный каталог. График игрока 187 снова открывается. Исходники расчёта не менялись.

- 2026-09-12, уточнение отображения: «Чётко» использует 12 шагов вместо 8,
  прозрачность края плавно снижается к нулю. Добавлен выбор «Фиолетовый / Светлый»
  для пересечения «Оба» без пересчёта поля. Инверсия работает только под пятнами.
  Целевые JS/PHP-проверки и проверка текущего отображения в браузере пройдены.
  Повторное измерение «Оба»: dust2 12,4–36,9 мс («Чётко») / 20,9–34,4 мс («Мягко»),
  as_oilrig 39,7–83,9 / 76,1–113,1 мс. Это время расчёта в Node по суммарным весам,
  без загрузки данных и отрисовки браузера. [Состав кандидата](release-heatmap-20260912.md).
