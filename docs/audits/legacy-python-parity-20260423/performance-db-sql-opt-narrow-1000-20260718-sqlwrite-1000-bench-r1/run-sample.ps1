...
try {
    ...
    & docker exec $name mysql -uroot -proot123 -D hlstatsxce --batch --raw -e 'SELECT "players" AS metric, COUNT(*) AS value FROM hlstats_Players UNION ALL SELECT "history",COUNT(*) FROM hlstats_Players_History UNION ALL SELECT "frags",COUNT(*) FROM hlstats_Frag_Frags UNION ALL SELECT "admin",COUNT(*) FROM hlstats_Events_Admin UNION ALL SELECT "statsme",COUNT(*) FROM hlstats_Events_Statsme UNION ALL SELECT "statsme2",COUNT(*) FROM hlstats_Events_Statsme2;' | Tee-Object -FilePath (Join-Path $audit "$Variant-$Sample-initial-anchors.tsv")
    Push-Location $Repo
    try {
        $env:PYTHONPATH = 'scripts;scripts\proxy_daemon_py'
        python scripts/replay_baseline/direct_import_artifacts.py --artifacts-dir $stage --configfile $config --work-dir (Join-Path $stage "$Variant-$Sample-work") --max-files 1000 --gs-ip 172.19.0.1 --gs-port 27015 --parser-backend native --stdin-transaction-batch-size 2500 --profile --audit-dir (Join-Path $audit "$Variant-$Sample-profile") --input-manifest (Join-Path $audit "$Variant-$Sample-input-manifest.txt") --ignored-lines-manifest (Join-Path $audit "$Variant-$Sample-ignored-lines.txt") 2>&1 | Tee-Object -FilePath $log -Append
