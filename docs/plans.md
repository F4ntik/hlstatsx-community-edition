...
Продолжение карточки игрока 12 сентября: исправлена форумная подпись и отступ под ником,
переведены действия в профиле, общем списке и карточке действия. График динамики
переведён на SVG с компактным прежним оформлением и значениями при наведении.
[Архитектура](charts-modern.md). Проверка ограничена затронутой веб-частью и упаковкой;
пересчёт журналов не требуется. Install/upgrade собираются из итогового коммита.

Точечное продолжение 12 сентября: исправление кириллицы и размещения текста
в форумных подписях `sig.php`; проверка на реальной подписи игрока в RU/EN.

Уточнение отображения 12 сентября: компактное сглаживание увеличено с 8 до 12 шагов,
край пятна плавно затухает; пересечение «Оба» можно сделать фиолетовым или светлым.
Инверсия действует под пятнами, оставляя остальную карту обычной. Целевые проверки
исходников, повторное измерение и проверка этого уточнения в браузере выполнены.
Автоматическая подготовка BSP сохранена. [Кандидат выпуска](release-heatmap-20260912.md)
собирается из окончательного коммита; точные хеши и подтверждение отправки ветки
фиксируются во внешней ведомости выпуска, без повторного прогона журналов.

Интеграция BSP-поверхностей 12 сентября: код Explorer, подготовленные данные всех 25 карт,
проверки PHP/JS и измерения готовы в отдельной экспериментальной ветке. План и граница
итоговой приёмки: [BSP heatmap integration](superpowers/plans/2026-09-12-bsp-heatmap-integration.md).
Локальные HTTP/браузерная проверка и независимый обзор завершены:
[результаты и ограничения](audits/bsp-heatmap-integration-20260912.md). Публичный выпуск не выполнялся.

Локальный эксперимент 12 сентября:
[BSP-маска и распространение по соседним поверхностям](audits/modern-heatmap-explorer/bsp-walls-20260912/README.md).
Автономное сравнение на de_dust2 создано в отдельной ветке. Решение об интеграции
в рабочий Explorer, реальные события и навигационная полнота остаются вне опыта.
