...
| `hlstats_Events_Entries` `legacy=0` vs `python>0` | Still rationalized for the current stage; fresh 2026-05-15 compare still shows `python=1017`, and the compare row remains intentionally visible. | Accepted legacy difference | Keep the compare line. Revisit only if entry rows are shown to damage visible stats or awards. |
